...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            # If the previous state is subset of the 'action' prenode, then we add this action
            if action_node.prenodes.issubset(self.s_levels[level]):
                actions_set.add(action_node)

            # Now we have to connect the State node to the new added action node
        ...
        self.a_levels.append(actions_set)
    ...
    def add_literal_level(self, level):
        ...
        # For each action node added in the last level of actions
        for action_node in self.a_levels[level-1]:

            # For each effect resulting from the action, we add a new literal node
            for effnode in action_node.effnodes:
                state_set.add(effnode)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # If the effects resulted from the action 1 is inside from the effects removed from the action 2
        if set(a1_action.effect_add).intersection(set(a2_action.effect_rem)):
            return True
        elif set(a2_action.effect_add).intersection(set(a1_action.effect_rem)):
            return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # If action 1 positive preconditions is inside the effects removed from the action 2
        if set(a1_action.precond_pos).intersection(set(a2_action.effect_rem)):
            return True
        elif set(a2_action.precond_pos).intersection(set(a1_action.effect_rem)):
            return True
        return False
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        # same symbol but different signal
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        # If none parent of the literal node 1 is mutex with any parent of the literal node 2
    ...
    def h_levelsum(self) -> int:
        ...
        # for each goal literal in the problem
        for goal in self.problem.goal:

            # set a flag to know if any goal state was found
            goal_found = False

            # iterate over the literals levels
            lv = 0
            for level in self.s_levels:
                for state in level:

                    # if the goal literal is found then add to the level_sum and go for the next literal
